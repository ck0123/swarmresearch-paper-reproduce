...
class BCSimulator:
    ...
    def initialization(self, path, config):
        ...
        # Reconstruct the authoritative network graph used for scoring so
        # candidate-provided edge metadata cannot spoof cost/throughput.
        self.reference_g = make_nx_graph(num_vms=int(self.default_vms_per_region))
        ...
        for dst in self.dsts:
            for partition_id in range(self.num_partitions):
                ...
                for edge in self.paths[dst][str(partition_id)]:
                    ...
                    if not self.reference_g.has_edge(src, dst):
                        raise ValueError(f"edge {src}->{dst} not in authoritative graph")
                    if not g.has_edge(src, dst):
                        true_edge_data = self.reference_g[src][dst]
                        cost = true_edge_data["cost"]
                        throughput = true_edge_data["throughput"]  # * self.default_vms_per_region
                        g.add_edge(src, dst, throughput=throughput, cost=cost, flow=throughput)
