# Findings — researcher-72

## Lens: acceptance-rate tax paid by the concrete high-uncertainty decision points

Deeper extension of the prior token-level decision-point locus. Prior analysis identified the
target verifier's high-entropy top-2 candidate forks as the dominant rejection region. This run
quantifies how much of the *total lost speculative acceptance* those positions account for, using
both realized lost draft slots and an expected lost-slot tax computed from the logged acceptance
probabilities.

### Method
- Reused the existing `results/content_<gpu>.jsonl` instrumentation: each draft slot has target
  entropy, target/draft truncated probabilities, rejection-sampling accept probability `ar`, and
  top candidate tokens.
- Added `analyze_acceptance_tax.py`. For each round, realized lost draft slots are `G - acc` and
  are attributed to the first rejected token. Expected lost-slot tax for draft slot `g` is
  `reach(g) * (1 - ar_g) * (G - g)`, where `reach(g)` is the product of earlier-slot acceptance
  probabilities in the same round. This charges the remaining aborted draft tail to the token-level
  decision point that would trigger it.
- Full benchmark `./evaluator/task-eval --gpus 4,5`: accuracy **93.3%**, acceptance **72.4%**,
  throughput **76.0 tok/s / 0.76x**. Throughput reflects instrumentation cost. Logs contain
  **65,830 rounds**, **263,320 drafted slots**, **190,764 accepted slots**, and **72,556 realized
  lost slots**. Expected lost slots from the logged `ar` values were **73,164**, matching realized
  loss closely.

### Core facts

**1. The acceptance tax is sharply concentrated in high-entropy verifier slots.**
Slots with **H >= 0.5** are **26.9% of drafted slots** but account for **82.5% of expected lost
slots** and **82.4% of realized lost slots**. The more extreme **H >= 1.0** slots are **13.2% of
drafted slots** but account for **48.2% expected / 48.3% realized** lost slots. Low-entropy slots
with **H < 0.5** are **73.1% of draft slots** but only **17.5% expected / 17.6% realized** tax.

**2. The prior pivotal decision-point set carries most of the tax at the H>=0.5 boundary.**
The **H >= 0.5 and pivotal** subset (discourse/numeric/content/mixed-class forks) is **19.0% of
drafted slots** and accounts for **59.1% of expected lost slots** and **59.0% of realized lost
slots**. The stricter **H >= 1.0 and pivotal** subset is **10.3% of drafted slots** and accounts
for **37.7% expected / 37.8% realized** lost slots. The **H >= 1.0 pivotal tight-binary** subset is
**5.0% of slots** and accounts for **18.2% expected / 18.1% realized** tax.

**3. Most low-level slots are effectively free; near-deterministic single-candidate slots are not
where acceptance is lost.**
`single` fork slots are **65.4% of all draft slots** but carry only **7.5% of expected lost slots**
and **7.6% of realized lost slots**. By fork type, the largest expected tax shares are
`content_alt` **31.9%**, `mixed_class` **21.1%**, `format` **15.1%**, `function_swap` **11.0%**,
and `discourse_fork` **10.6%**. Pivotal fork types together carry **65.1%** of expected tax.

**4. The tax is multiplicative with draft depth because early rejections abort the tail.**
Slot 0 carries **47.0%** of expected lost slots and **46.9%** of realized lost slots; slot 1 carries
**29.5% / 29.3%**; slot 2 carries **16.3% / 16.5%**; slot 3 carries **7.1% / 7.3%**. The mean lost
draft slots per first rejection is **2.70 realized** and **2.71 expected**.

**5. Individual-slot concentration is high even before grouping by semantic type.**
The top **1%** of draft slots by expected tax carry **13.5%** of expected lost slots; top **2%**
carry **24.2%**; top **5%** carry **48.1%**; top **10%** carry **72.7%**; top **20%** carry
**94.4%**. Realized first-rejection tail loss is less concentrated by tail length alone: the top
20% of realized first rejections carry **29.6%** of realized lost slots.

**6. Concrete recurring pair identities are individually small, but their classes are large.**
The largest concrete non-single expected-tax pairs are formatting/math/prose/function forks:
`\n`|`\n\n` **0.5%**, ` the`|` $` **0.5%**, ` $`|` the` **0.5%**, `\n\n`|`\n` **0.4%**. The
continue-vs-backtrack pair `Let`|`Wait` carries **0.3%** expected tax, and `Wait`|`Let` carries
**0.2%**. The top 40 pair-ranking rows, including the aggregate `<single>` row, carry **15.9%** of
expected tax, while the aggregate fork classes above explain the majority.

**7. The high-entropy pivotal-tax share is stable across non-math domains and lower in AIME.**
Within-domain share of expected lost-slot tax from **H >= 1.0 pivotal** slots: aime **29.2%**,
gpqa **38.6%**, hle **39.3%**, lcb **39.2%**. Total expected-tax share by domain was lcb
**32.9%**, gpqa **27.8%**, hle **26.0%**, aime **13.3%**.

### Artifacts
- `analyze_acceptance_tax.py` — acceptance-loss accounting by entropy, fork type, draft depth,
  individual-slot concentration, concrete fork pairs, and domain.
- `results/acceptance_tax_summary.txt` — full numeric output.
- `results/uncertainty_positions_summary.txt`, `results/content_4.jsonl`,
  `results/content_5.jsonl`, `results/task_eval.json` — regenerated on GPUs 4,5.
