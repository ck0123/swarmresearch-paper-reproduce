...
# Extension 3 — the break-even serving batch: when speculation stops paying, measured in the combined regime

## Angle
Extension 2 measured the two amortization axes **in isolation** (cross = B seqs × 1 token;
within = 1 seq × W tokens) and concluded the spec-dec premise "erodes as batch grows." But a real
server runs **both at once**: B concurrent requests, each verifying W=γ+1 speculative tokens — a
2-D batch of B·W tokens carrying cross-sequence fan-out (B decorrelated prefixes) AND
within-sequence fan-out (W correlated continuations) *simultaneously*. The qualitative "erodes"
claim was never quantified. This extension measures the combined forward directly and turns the
reframing into a decision: **the break-even serving batch B\* beyond which adding speculative
decoding reduces throughput**, and the closed inequality that governs it.

## Method — new `--mode servspec` (isolated branch, non-perturbing)
New isolated `--mode servspec` in `initial_program.cpp`; scored `target`/`mtp-spec` paths are
byte-for-byte unchanged (smoke: mtp-spec **81.3% accept / 1.59×**; full scored eval below).
Single-GPU (CUDA_VISIBLE_DEVICES=2), medians of 20–30 reps after 8 warmups, on real prefilled
prompts from the 30-item set, distinct per-sequence prefixes (realistic concurrent requests).
- **Combined verify forward C_tgt(B,W):** B sequences prefilled to L, then one forward over B·W
  query tokens (W consecutive positions/seq), every position given a 262K logit row (real spec
  verify). `B`∈{1..256} (powers of 2, capped by llama's 256-seq limit), `W`∈{1,2,3,5} (=γ+1 for
  γ∈{0,1,2,4}). High-B sweep uses L=64 to fit KV (B·(L+5) ≤ n_ctx=24576); B≤64 cross-checked at L=256.
- **Batched draft C_dft(B):** width-B MTP forward (B sequences × 1 token, B 262K rows) = one draft
  step of a B-request server.
- **Throughput model** (GPU-forward time; CPU sampling excluded — see caveats):
  - no-spec server:  `tput(B)   = B / C_tgt(B,1)`
  - spec server:     `tput(B,γ) = B·E_acc(γ) / (γ·C_dft(B) + C_tgt(B,γ+1))`
  - spec/no-spec speedup `S(B,γ) = E_acc(γ)·C_tgt(B,1) / (γ·C_dft(B)+C_tgt(B,γ+1))`; S<1 ⇒ spec hurts.
- **E_acc(γ)** = expected emitted tokens/request/round = 1 + Σ_{i=1..γ} αⁱ (i.i.d. accept-until-reject),
  with per-position α fit to the **measured** scored yield (below); α=0.876 ⇒ E_acc = 1.88/2.64/3.90 for γ=1/2/4.

## Measured anchors (full scored mtp-spec eval, GPUs 2,3, γ=4)
**93.3% accuracy (lossless, = target), 148.9 tok/s, 72.4% accept, 1.48× speedup.** Aggregated from
the raw TIMING lines (65 830 rounds): drafted/round=4.0, **accepted/round=2.90, tokens/round=3.90**,
**verify_fwd/round=18 138 µs**, draft_fwd/step=1 141 µs. The probe reproduces these at B=1:
C_tgt(1,5)=17 381 µs ≈ measured 18 138 µs (✓ within 4%); C_dft(1)=1 109 µs ≈ 1 141 µs.

## Result S1 — spec stays profitable far past the prior "erodes" implication, then crosses HARD
S(B,γ) (GPU-forward; >1 = spec wins). R5 = C_tgt(B,5)/C_tgt(B,1) is the γ=4 verify-width cost ratio.

| B | nospec tok/s | R5 | S(γ=1) | S(γ=2) | S(γ=4) |
|---|---|---|---|---|---|
| 1 | 110 | 1.92 | 1.40 | 1.51 | **1.62** |
| 8 | 300 | 1.38 | 1.59 | 1.90 | **2.24** (peak) |
| 32 | 827 | 1.89 | 1.35 | 1.44 | 1.73 |
| 64 | 1179 | 2.11 | 1.15 | 1.45 | 1.52 |
| 128 | 1440 | 2.79 | 1.34 | 1.46 | **1.17** |
| 256 | 2076 | 3.85 | 1.18 | **0.94** | **0.83** |

- Spec does **not** collapse at modest server batch: γ=4 holds **1.5–2.2×** through B≤64 and is still
  +17% at B=128. The B=1 model speedup (1.62×) overshoots the scored 1.48× only by the omitted CPU
  term (see caveats), so the model is an **upper bound** on spec's benefit.
- It then crosses hard. Directly bracketed (interp where S=1): **γ=4 → B\*≈182** (S: 1.17 at 128 →
  **0.83 at 256**, a 17% throughput *loss*); **γ=2 → B\*≈237**; **γ=1 → B\*>256**. Because CPU sampling
  (excluded) penalizes spec's B·(γ+1) make_dist calls more than no-spec's B, the true crossovers are
  **earlier** than these forward-only B\*.
- **More aggressive speculation crosses earlier** (γ=4 at ~182 vs γ=1 past 256): optimal draft depth
  is **non-increasing in serving batch** — at high batch, shallow or no speculation wins.

## Result S2 — the crossover is exactly where the verify-width ratio meets the token yield
Spec helps iff `C_tgt(B,γ+1) + γ·C_dft(B) < E_acc(γ)·C_tgt(B,1)`, i.e. iff
`R_{γ+1}(B) < E_acc(γ) − γ·C_dft(B)/C_tgt(B,1)`. The ratio **R5 climbs 1.92→3.85** as B grows
(fixed cost amortizing away makes each extra verify token cost its full marginal), and the γ=4
crossover lands exactly where **R5 (3.85 at B=256) meets E_acc(4)=3.90**. The reframing's "fixed cost
gets amortized by batch, so spec tokens stop being free" is thus a *quantitative* statement: spec
dies when widening the verify forward by (γ+1) costs ≥ E_acc(γ)× a unit forward.

## Result S3 — mechanism: three measured terms move against spec as B grows
| B | nospec µs/tok | spec-token marginal µs/tok (within-seq) | draft phase γ·C_dft/C_tgt(B,1) |
|---|---|---|---|
| 1 | 9049 | 2083 | 0.49 |
| 64 | 848 | 236 | 0.45 |
| 256 | 482 | 343 | 0.84 |

- **Numerator shrinks:** an accepted spec token is worth the baseline forward it replaces; that worth
  falls 9049→482 µs/tok (18×) as the fixed cost amortizes. The spec-token marginal (within-sequence
  MoE fan-out) settles to ~240–340 µs/tok — no longer ≪ the baseline per-token cost at high B, so the
  "nearly-free extra verify token" premise is gone.
- **Draft denominator grows (relatively):** the once-tiny batched draft eats 0.49→**0.84** of a full
  baseline forward by B=256. Batched drafting amortizes its LM-head fixed read (per-token draft
  1109→101 µs) but its floor (262K-row LM-head compute + MoE marginal) is a *larger fraction* of the
  also-falling baseline per-token cost — connecting to prior E3 (draft = 57% shared LM head): that
  shared head, cheap to amortize at B=1, is paid per-token by all B requests at high B.

## Unifying picture (closes the Extension-2 reframing)
Extension 2 cast speculative depth and request batching as *substitutes* for one fixed cost and left
"spec erodes with batch" qualitative. The combined-regime measurement makes it quantitative and
**inverts the deployment intuition twice**: (1) spec is *more* robust to batching than "substitutes"
implies — the fixed cost is so large that B≤64–128 only partly amortizes it, so spec keeps a real
1.2–2.2× there; (2) but it crosses to a net **tax** at a concrete, measured batch (γ=4: B\*≈180,
forward-only upper bound; earlier with CPU cost), and deeper speculation crosses sooner. Speculative
decoding is a **low-batch / latency-bound** optimization whose benefit is bounded by the verify-width
cost ratio R_{γ+1}(B) overtaking the token yield E_acc(γ); in throughput-bound (high-batch) serving
the same MoE expert-fan-out marginal that capped single-sequence speedup (~2.1×) now flips spec
negative.

## Measurement caveats (Extension 3)
- **GPU-forward only.** The model omits CPU sampling (make_dist) and cache ops (~12.8% of wall-clock
  per the Result-1 decomposition). Spec issues B·(γ+1) verify + B·γ draft make_dist calls/round vs
  no-spec's B; including it lowers every S and moves B\* **earlier** — the reported B\* are upper bounds.
- **i.i.d. acceptance.** E_acc(γ) uses one per-position α fit to the measured γ=4 yield; real per-depth
  acceptance declines with depth, so E_acc for γ≠4 is approximate (over-credits deep drafts → again
  optimistic for spec).
- **Per-cell timing noise / CUDA tiling.** Some (B,W) cells are non-monotonic (e.g. B=4 W2>W3), the
  kernel-tiling artifact noted in Extension 2; the S(γ=4) trend is monotone from B=8 and the B=256
  crossing (0.83) is unambiguous.
- **L=64 for the high-B sweep** (KV fit); B≤64 reproduced at L=256 (W5: 114.7k vs 119.5k µs), and
  prior B3 showed the structure is largely L-invariant. Single-GPU data-parallel; n_seq_max capped at
  llama's 256. Draft content held fixed (one seed hidden) since draft cost is content-independent at
  fixed width; if hp for the seed is unavailable under multi-seq config it falls back to zeros (same cost).
