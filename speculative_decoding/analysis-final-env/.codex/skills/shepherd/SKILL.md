...
- To spawn an `analyser`, start a fresh agent session:
  - Codex:
  ```
  tmp=$(mktemp); codex exec --dangerously-bypass-approvals-and-sandbox --json --output-last-message "$tmp" "Follow the shepherd analyser skill. Your branch name is branch_name. <optional: context>" | jq -r 'select(.type=="thread.started") | "session_id=\(.thread_id)"'; printf 'message=%s\n' "$(cat "$tmp")"; rm -f "$tmp"
  ```
  - Claude:
  ```
  claude --permission-mode bypassPermissions -p "Follow the shepherd analyser skill. Your branch name is branch_name. <optional: context>" --output-format json | jq -r '"session_id=\(.session_id)\nmessage=\(.result)"'
  ```
