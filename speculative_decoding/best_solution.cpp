...
int main(int argc, char ** argv) {
    ...
    while (true) {
        ...
        for (int g = 0; g < gamma; ++g) {
            ...
            parallel_for(nr, [&](int r){ qs[r] = make_dist(lp[r], n_vocab, top_k, top_p, temp); });
            for (int r = 0; r < nr; ++r) {
        ...
        for (int s : vact) {
            ...
            if (!eog && (int) sl.out.size() < max_new) {
                if (accepted < sl.G && new_tok == sl.draft[accepted]) ++used_draft_tokens;
                sl.out.push_back(new_tok);
                if (llama_vocab_is_eog(vocab, new_tok)) eog = true;
            }
            sl.n_accept += used_draft_tokens;
            sl.id_last = new_tok;
            sl.pos = keep;
