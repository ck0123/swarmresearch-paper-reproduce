# Direct support repair (researcher-60) — mix copy support into the MTP proposal

## Angle
The prior branch result was that the dominant MTP chain failure is not draft-token rank
misordering but **support miss**: at chain breaks, the target's realized token is outside the MTP
proposal support 76.4% of the time. This extension tests a direct intervention at the proposal
level: add an external support source at each speculative depth and measure whether those newly
supported tokens become accepted tokens and improve wall-clock throughput.

## Method
Added a lossless decoder mode, **`mtp-copy-mix`**, that keeps the normal MTP sequential draft but
changes each draft-depth proposal distribution from `q_mtp` to:

`q_mix = (1-alpha) * q_mtp + alpha * point_mass(copy_candidate)`

where `copy_candidate` is the same longest-suffix prompt-lookup candidate used by the earlier
copy-index tooling, evaluated over prompt + confirmed generation + already drafted tokens in the
current round. Verification uses this exact mixed `q_mix` in the existing rejection sampler, so the
mode remains distribution-preserving for the target sampling distribution. The full benchmark used
`COPY_MIX_ALPHA=0.05`, `gamma=4`, and GPUs 4,5:

`COPY_MIX_ALPHA=0.05 ./evaluator/task-eval --modes mtp-spec,mtp-copy-mix --gpus 4,5`

Instrumentation (`MIXSUPPORT`) records how often a copy candidate exists, how often it is outside
native MTP support, how often the mixed sampler actually draws an outside-MTP-support token, and how
often such a token is accepted. Aggregation tooling: `analyze_mix.py`.

## Result M1 — the intervention widens support but does not improve throughput

Full 30-item eval:

| mode | accuracy | tok/s | accept | speedup |
|---|---:|---:|---:|---:|
| target reference | 93.3% | 100.7 | — | 1.00× |
| mtp-spec | 93.3% | 150.8 | 72.4% | 1.50× |
| **mtp-copy-mix (alpha=0.05)** | **90.0%** | **148.2** | **71.0%** | **1.47×** |

Raw aggregate counters:

| mode | decoded | drafted | accepted | rounds | accepted/round | verify ms/round |
|---|---:|---:|---:|---:|---:|---:|
| mtp-spec | 256,580 | 263,320 | 190,764 | 65,830 | 2.898 | 18.136 |
| mtp-copy-mix | 248,122 | 258,460 | 183,528 | 64,615 | 2.840 | 18.130 |

The mixed proposal has the same verify cost per round as the baseline, but lower accepted tokens per
round. The end-to-end result is slightly slower: 148.2 tok/s vs 150.8 tok/s in the evaluator's
per-item mean (149.6 vs 151.4 aggregate decoded-token/time from raw timings). The 90.0% vs 93.3%
accuracy difference is one item on a stochastic 30-item run; this mode changes the sample path but
uses the same target distribution via rejection sampling.

## Result M2 — copy provides many outside-support candidates, but they rarely become useful drafts

`mtp-copy-mix` support counters over the full run:

| counter | count | rate |
|---|---:|---:|
| copy candidate present | 241,405 | 93.4% of drafts |
| copy candidate outside native MTP support | 109,928 | 45.5% of copy candidates |
| outside-MTP-support token sampled | 5,569 | 2.15% of drafts |
| outside-MTP-support token accepted | 429 | 7.7% of outside samples; 0.23% of all accepts |

The intervention directly addresses the diagnosed support miss in the mechanical sense: it places
nonzero proposal mass on 109,928 candidate tokens that native MTP would not have proposed, and 429
accepted tokens came from outside native MTP support. The effect is too small to matter end-to-end:
the outside-support accepted tokens are only 0.23% of all accepted draft tokens, while the copied
support mass also perturbs the high-quality native MTP proposal enough to reduce overall acceptance
from 72.4% to 71.0%.

## Integrated conclusion
The support-miss diagnosis survives an active repair attempt. Adding a cheap external support source
at every MTP depth does create a measurable number of accepted tokens that native MTP could not have
accepted, but the precision and sampling rate of those repaired-support tokens are far below the
level needed to improve verify-width-per-accepted-token. In this experiment, proposal support is not
the only bottleneck: the added support must also be accurate enough, and carry enough proposal mass,
to replace rather than dilute the MTP head's already strong top-1 behavior.
